...
if [ "$TRAVIS_PULL_REQUEST" != "false" ]; then
  ...
  ./gradlew -Prelease.useLastTag=true -Dsonar.login=$SONAR_TOKEN -Dsonar.projectVersion=$TRAVIS_BRANCH -Dsonar.analysis.mode=preview -Dsonar.github.pullRequest=$TRAVIS_PULL_REQUEST -Dsonar.github.repository=$TRAVIS_REPO_SLUG -Dsonar.github.oauth=$GITHUB_ACCESS_TOKEN build sonarqube --info
